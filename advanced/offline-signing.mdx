---
title: "Offline Transaction Signing and Raw Wire Encoding"
...
|---|---|---|
| `fetchChainId` | `(rpcUrl: string) => Promise<Uint8Array>` | Resolve a 32-byte chain ID from CometBFT `/status` over the network |
| `chainIdFromString` | `(chainId: string) => Uint8Array` | Hash a chain ID string to 32 bytes without a network call |
| `signAndEncode` | `(chainId, action, seq, privateKey) => Uint8Array` | Build a complete signed wire envelope |
| `decodeTx` | `(bytes: Uint8Array) => { version, action, seq, pubkey, signature }` | Decode and verify the structure of a wire envelope |
| `encodePayloadBytes` | `(action: Action) => Uint8Array` | Encode only the action payload, with no signature wrapper |
| `peekActionType` | `(bytes: Uint8Array) => ActionTypeValue \| null` | Read the action type byte without a full decode |
| `signEnvelopeFromPayload` | `(chainId, actionType, seq, payloadBytes, privateKey) => Uint8Array` | Sign and wrap pre-encoded payload bytes — useful for cross-language conformance |
